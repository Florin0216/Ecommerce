...
import ShippingCreateDto from "../../dto/Shipping/ShippingCreateDto";
import ShippingEditDto from "../../dto/Shipping/ShippingEditDto";
...
const shipping = ref(JSON.parse(JSON.stringify(props.shipping)));

const isNewShipping = computed(() => !shipping.value.id);

const onConfirm = () => {
    const promise = isNewShipping.value
        ? ShippingService.new(new ShippingCreateDto({
            ...shipping.value,
            user: props.user.id
        }))
        : ShippingService.edit(shipping.value.id, new ShippingEditDto(shipping.value));
    ...
    promise
        .then((response) => {
            ...
            ToastService.show(
                isNewShipping.value
                    ? "Shipping details created successfully!"
            ...
            <div class="relative bg-white rounded-lg shadow-sm">
                <div class="flex items-center justify-between p-4 md:p-5 border-b border-gray-400">
                    <h3 class="text-xl font-semibold text-gray-900">
                        <template v-if="isNewShipping">Add shipping</template>
                        <template v-else>Shipping details</template>
                    </h3>
                    <button
                        @click="dismissModal"
                        type="button"
                ...
                <div class="p-4 md:p-5 space-y-4">
                    <form class="space-y-6">
                       ...
                       dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                            >
                                <template v-if="isNewShipping">Add</template>
